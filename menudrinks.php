...
{
	if(isset($_SESSION["drinks_cart"]))
	{
		$item_array_id = array_column($_SESSION["drinks_cart"], "item_id");
		if(!in_array($_GET["id"], $item_array_id))
		{
			$count = count($_SESSION["drinks_cart"]);
			$item_array = array(
				'item_id'			=>	$_GET["id"],
			...
			$_SESSION["drinks_cart"][$count] = $item_array;
		}
		else
		{
	...
	else
	{
		...
		$_SESSION["drinks_cart"][0] = $item_array;
	}
}
...
if(isset($_GET["action"]))
{
	if($_GET["action"] == "delete")
	{
		foreach($_SESSION["drinks_cart"] as $keys => $values)
		{
			...
			{
				unset($_SESSION["drinks_cart"][$keys]);
				echo '<script>alert("Item Removed")</script>';
				echo '<script>window.location="menudrinks.php"</script>';
			}
		}
	}
}
  ...
  left: 40%;">Order a drink !</h6><br /> 
	<body>
		...
		<div class="container">
			...
			<br /><br />
			<?php
				$query = "SELECT * FROM menudrinks ORDER BY id ASC";
				...
				?>
			<div class="col-md-4">
				<form method="post" action="menudrinks.php?action=add&id=<?php echo $row["id"]; ?>">
					<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;" align:center>
						<img src="images/drinks/<?php echo $row["image"]; ?>" class="img-responsive" /><br />
			...
			<?php
             ...
             color: #2F3841;">Sorry but there is no data to show you can wait for the admin to add more content</h3>
                            ...
                            <img  src="images/errors/errordrinks.jpg" class="img-responsive" /><br />
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					<?php
                            ...
                            $_SESSION['total_price_drinks'] = $total;
						}
					?>
					<tr>
