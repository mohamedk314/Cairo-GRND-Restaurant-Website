...
{
	if(isset($_SESSION["dinner_cart"]))
	{
		$item_array_id = array_column($_SESSION["dinner_cart"], "item_id");
		if(!in_array($_GET["id"], $item_array_id))
		{
			$count = count($_SESSION["dinner_cart"]);
			$item_array = array(
				'item_id'			=>	$_GET["id"],
			...
			$_SESSION["dinner_cart"][$count] = $item_array;
		}
		else
		{
	...
	else
	{
		...
		$_SESSION["dinner_cart"][0] = $item_array;
	}
}
...
if(isset($_GET["action"]))
{
	if($_GET["action"] == "delete")
	{
		foreach($_SESSION["dinner_cart"] as $keys => $values)
		{
			...
			{
				unset($_SESSION["dinner_cart"][$keys]);
				echo '<script>alert("Item Removed")</script>';
				echo '<script>window.location="menudinner.php"</script>';
			}
		}
	}
}
  ...
  left: 40%;">Order Dinner !</h6><br /> 
	<body>
		...
		<div class="container">
			...
			<br /><br />
			<?php
				$query = "SELECT * FROM menudinner ORDER BY id ASC";
				...
				?>
			<div class="col-md-4">
				<form method="post" action="menudinner.php?action=add&id=<?php echo $row["id"]; ?>">
					<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;" align:center>
						<img src="images/dinner/<?php echo $row["image"]; ?>" class="img-responsive" /><br />
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					<?php
                            ...
                            $_SESSION['total_price_dinner'] = $total;
						}
					?>
					<tr>
